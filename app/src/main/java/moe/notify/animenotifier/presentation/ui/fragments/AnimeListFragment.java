...
public class AnimeListFragment extends Fragment implements AnimeListPresenter.View {
  ...
  boolean useGridView;
  @BindView(R.id.animeList) RecyclerView mAnimeListRecyclerView;
  @BindView(R.id.loadingTextView) TextView mLoadingTextView;
  @BindView(R.id.loadingSpinner) ProgressBar mLoadingSpinner;
  ...
  private void init() {
    ...
    if (useGridView) {
      mLayoutManager = new GridLayoutManager(mContext,
          (getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) ? 2
              : 4);
    } else {
    ...
    mAnimeListPresenter = new AnimeListPresenterImpl(ThreadExecutor.getInstance(),
        MainThreadImpl.Companion.getInstance(), this, new AnimeListRepositoryImpl(getContext()) {
    });
  }
  ...
  @Override public void showAnimeList(AnimeList animeList) {
    // signal the adapter that it has data to show
    adapter.addNewAnimes(animeList);
  }

  @Override public void onClickDownload(long animeId, int position) {
  ...
  @Override public void onClickAnime(long animeId) {
